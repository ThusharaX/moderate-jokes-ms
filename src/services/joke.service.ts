...
const updateSubmittedJoke = async (id: string, data: Partial<{ setup: string; punchline: string; type: string; author: string }>) => {
  // ! api request to 'submit-jokes-ms' microservice
...
const approveJoke = async (id: string, data: { setup: string; punchline: string; type: string; author: string }) => {
  // ! api request to 'deliver-jokes-ms' microservice - create joke
  // ! api request to 'submit-jokes-ms' microservice - delete joke
  const createJokeResponse = await axios.post(`${DELIVER_JOKES_MS}/jokes`, data);

  if (createJokeResponse.status === 201) {
    const deleteJokeResponse = await axios.delete(`${SUBMIT_JOKES_MS}/jokes/${id}`);
...
export { getAllSubmittedJokes, getSubmittedJokeById, updateSubmittedJoke, deleteSubmittedJoke, getJokeTypes, approveJoke, rejectJoke };